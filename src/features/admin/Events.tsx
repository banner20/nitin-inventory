...
function EventRow({
  ...
  return (
    <div className="p-3 flex flex-wrap items-center justify-between gap-3 hover:bg-surface-hover transition-colors">
      {/* The name is the way in — one event's costs live on their own page,
          not behind a download. */}
      <Link to={`/admin/events/${event.id}`} className="min-w-0 flex-1 group">
        <p className="font-medium text-sm truncate group-hover:text-brand-600 transition-colors">
          {event.name}
